...
public class Baseline {
    ...
    public void CreateMAC(boolean isMAC) {
        if (isMAC) {
            ...
            for (int i = 0; i < num_bits_per_row; i++) {
                ...
                MACs[i] = new byte[Nonce_prf.PRFBYTES];
                for (int j = 0; j < doc_size; j++) {
                    ...
                    MACs[i] = ByteUtils.xor(MACs[i], nonceprf.create(Utils.stringToBits(data)));
                }
            }
            end = System.nanoTime();
        ...
        for (int i = 0; i < inputs.length; i++) {
            ...
            for (int j = 0; j < doc_size; j++) {
                ...
                if (isMAC) {
                    ...
                    test_mac = ByteUtils.xor(test_mac, nonceprf.create(Utils.stringToBits(data)));
                }

            }
            if (isMAC) {
                if (!nonceprf.equal(test_mac, query_mac)) {
    ...
    public void UpdateSim(boolean isMAC) {
        ...
        if (isMAC) {
            for (int j = 0; j < num_bits_per_row; j++) {
                ...
                update_macs[j] = ByteUtils.xor(update_macs[j], nonceprf.create(Utils.stringToBits(data)));
            }
    ...
    public static void main(String[] args) {
        ...
        path = "/home/Less_project-update/Less_project-main/src/main/java/org/example/dataset/synthetic_128_100.csv";
        max_size = 128;
        doc_size = 10;
        mode = 2;
